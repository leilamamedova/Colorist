...
searchBox.addEventListener('click', (e) => {
  ...
  document.getElementById('hex_input').classList.add("input");
...
document.addEventListener('click', function(event) {
  var isClickInsideElement = searchBox.contains(event.target);
  if (!isClickInsideElement) {
    if(document.getElementById('hex_input').value == '#' || document.getElementById('hex_input').value == '') {
      document.getElementById('hex_input').value = '#D4EDFB';
      Colorist(hash='#', "D4EDFB");
    }
  }
});
